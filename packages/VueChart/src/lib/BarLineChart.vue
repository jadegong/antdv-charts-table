...
export default {
    ...
    mounted() {
        if (this.option) {
            barLineChart = echarts.init(this.$refs.domChart, this.option.theme === 'dark' ? dark : light);
    ...
    methods: {
        ...
        setBarLineOption(option) {
                              ...
                              normal: {
                                ...
                                formatter(a) {
                                  if (opts.markPointFormatter) {
                                    return eval(opts.markPointFormatter).call(this, a.value, toolUtil);
                                  }
            ...
            if (opts.optionBarLine.seriesLine) {
                toolUtil.extend(barLineOption.seriesLine, opts.optionBarLine.seriesLine);
            }
            ...
            if (data && data.length > 0) {
              ...
              if (barName.length > 1) {
                barName.forEach((item, index) => {
                  barLineOption.series[index] = toolUtil.extend({}, barLineOption.seriesBar);
                ...
                barLineOption.series[0] = toolUtil.extend({}, barLineOption.seriesBar);
              ...
              if (opts.barColor.length > 1) {
                opts.barColor.forEach((item, index) => {
                  barLineOption.series[index].itemStyle.normal.color = opts.barColor[index];
                });
              } else {
              ...
              if (opts.lineColor.length > 1) {
                if (opts.barColor.length > 1) {
                  opts.barColor.forEach((item, index) => {
                    barLineOption.series[opts.barColor.length + index].lineStyle.normal.color =
                      opts.lineColor[index];
                    barLineOption.series[opts.barColor.length + index].itemStyle.normal.color =
                      opts.lineColor[index];
                  });
                } else {
                  opts.lineColor.forEach((item, index) => {
                    barLineOption.series[index + 1].lineStyle.normal.color = opts.lineColor[index];
                    barLineOption.series[index + 1].itemStyle.normal.color = opts.lineColor[index];
                  });
                }
              } else if (opts.barColor.length > 1) {
                barLineOption.series[opts.barColor.length].lineStyle.normal.color = opts.lineColor[0];
                barLineOption.series[opts.barColor.length].itemStyle.normal.color = opts.lineColor[0];
              } else {
                ...
                if (lineName.length > 1) {
                  if (barName.length > 1) {
                    lineName.forEach((lineItem, indexB) => {
                      barLineOption.series[barName.length + indexB].data.push(item[lineName[indexB]]);
                    });
                  } else {
                    lineName.forEach((lineItem, indexB) => {
                      barLineOption.series[indexB + 1].data.push(item[lineName[indexB]]);
                    });
                  }
                ...
                let temp2 = [];
                  ...
                  barLineOption.series.forEach((singleSeries, singleSeriesIndex) => {
                      singleSeries.data.forEach((singleDataVal) => {
                          if (singleSeriesIndex < barName.length) { // 获取bar类型最大最小值
                              min1 = (min1 === undefined || min1 > singleDataVal) ? singleDataVal : min1;
                              max1 = (max1 === undefined || max1 < singleDataVal) ? singleDataVal : max1;
                          } else { // 获取bar类型最大最小值
                              min2 = (min2 === undefined || min2 > singleDataVal) ? singleDataVal : min2;
                              max2 = (max2 === undefined || max2 < singleDataVal) ? singleDataVal : max2;
                          }
                      });
                  });
